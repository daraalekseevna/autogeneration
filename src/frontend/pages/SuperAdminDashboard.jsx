...
const SuperAdminDashboard = () => {
  ...
  const [newTeacher, setNewTeacher] = useState({
    ...
    subjectIds: [], // массив id выбранных предметов
    login: '', password: ''
  });
  ...
  const loadData = async () => {
    ...
    try {
      const [adminsRes, teachersRes, classesRes, subjectsRes] = await Promise.all([
        ...
        axios.get('/api/superadmin/teachers', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('/api/superadmin/classes', { headers: { Authorization: `Bearer ${token}` } }),
        axios.get('/api/superadmin/subjects', { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const handleAddTeacher = async (e) => {
    ...
    try {
      await axios.post('/api/superadmin/teachers', newTeacher, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteTeacher = async (id) => {
    ...
    try {
      await axios.delete(`/api/superadmin/teachers/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAddClass = async (e) => {
    ...
    try {
      await axios.post('/api/superadmin/classes', newClass, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleChangeShift = async (classId, currentShift) => {
    ...
    try {
      await axios.put(`/api/superadmin/classes/${classId}`, { shift: newShift }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleToggleTeacher = async (classId, currentTeacherId) => {
    try {
      if (currentTeacherId) {
        // Отвязать
        await axios.put(`/api/superadmin/classes/${classId}`, { teacherId: null }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        showNotification('Классный руководитель отвязан');
      } else {
        // Назначить первого попавшегося учителя (упрощённо – можно добавить выбор)
        if (teachers.length === 0) {
          showNotification('Нет учителей для назначения');
          return;
        }
        await axios.put(`/api/superadmin/classes/${classId}`, { teacherId: teachers[0].id }, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleDeleteClass = async (id) => {
    ...
    try {
      await axios.delete(`/api/superadmin/classes/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    <div className="main-content-page">
      ...
      <main className="superadmin-container">
        ...
        <div className="tabs-container">
          <button onClick={() => setActiveTab('admins')} className={`tab-button ${activeTab === 'admins' ? 'active' : ''}`}>Администраторы</button>
          <button onClick={() => setActiveTab('teachers')} className={`tab-button ${activeTab === 'teachers' ? 'active' : ''}`}>Учителя</button>
          <button onClick={() => setActiveTab('classes')} className={`tab-button ${activeTab === 'classes' ? 'active' : ''}`}>Классы</button>
        </div>
              ...
              <div className="form-container">
                ...
                <form onSubmit={handleAddAdmin}>
                  <div className="form-group">
                    ...
                    <input type="text" value={newAdmin.login} onChange={(e) => setNewAdmin({...newAdmin, login: e.target.value})} className="form-input" required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Пароль *</label>
                    <input type="password" value={newAdmin.password} onChange={(e) => setNewAdmin({...newAdmin, password: e.target.value})} className="form-input" required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">ФИО *</label>
                    <input type="text" value={newAdmin.name} onChange={(e) => setNewAdmin({...newAdmin, name: e.target.value})} className="form-input" required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Email *</label>
                    <input type="email" value={newAdmin.email} onChange={(e) => setNewAdmin({...newAdmin, email: e.target.value})} className="form-input" required />
                  </div>
                    ...
                    <tbody>
                      {admins.map(a => (
                        <tr key={a.id}>
                          ...
                          <td>{a.email}</td>
                          <td>
                            <button onClick={() => handleDeleteAdmin(a.id)} className="action-button delete-button"><FaTrash /> Удалить</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="form-container">
                ...
                <form onSubmit={handleAddTeacher}>
                  <div className="form-group">
                    ...
                    <input type="text" value={newTeacher.lastName} onChange={e => setNewTeacher({...newTeacher, lastName: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Имя *</label>
                    <input type="text" value={newTeacher.firstName} onChange={e => setNewTeacher({...newTeacher, firstName: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Отчество</label>
                    <input type="text" value={newTeacher.middleName} onChange={e => setNewTeacher({...newTeacher, middleName: e.target.value})} />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Логин *</label>
                    <input type="text" value={newTeacher.login} onChange={e => setNewTeacher({...newTeacher, login: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Пароль *</label>
                    ...
                    <tbody>
                      {teachers.map(t => (
                        <tr key={t.id}>
                          ...
                          <td>
                            <button onClick={() => handleDeleteTeacher(t.id)} className="action-button delete-button"><FaTrash /> Удалить</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            <div className="content-grid">
              <div className="form-container">
                ...
                <form onSubmit={handleAddClass}>
                  <div className="form-group">
                    ...
                    <input type="number" min="1" max="11" value={newClass.number} onChange={e => setNewClass({...newClass, number: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Буква *</label>
                    <input type="text" maxLength="1" value={newClass.letter} onChange={e => setNewClass({...newClass, letter: e.target.value.toUpperCase()})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Логин *</label>
                    <input type="text" value={newClass.login} onChange={e => setNewClass({...newClass, login: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Пароль *</label>
                    <input type="password" value={newClass.password} onChange={e => setNewClass({...newClass, password: e.target.value})} required />
                  </div>
                  <div className="form-group">
                    ...
                    <select value={newClass.shift} onChange={e => setNewClass({...newClass, shift: parseInt(e.target.value)})}>
                      <option value="1">1 смена</option><option value="2">2 смена</option>
                    </select>
                  </div>
                  <div className="form-group">
              ...
              <div className="table-container">
                        ...
                        <tr key={c.id}>
                          ...
                          <td>
                            <div style={{display:'flex', gap:'0.5rem'}}>
                              ...
                              <button onClick={() => handleChangeShift(c.id, c.shift)} className="action-button shift-button">Сменить</button>
                            </div>
                          </td>
                          ...
                          <td>
                            <button onClick={() => handleDeleteClass(c.id)} className="action-button delete-button"><FaTrash /> Удалить</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="shift-stats">
                  <div className="shift-stat"><div className="shift-stat-number shift-stat-1">{stats.shift1}</div><div>Классов в 1 смену</div></div>
                  <div className="shift-stat"><div className="shift-stat-number shift-stat-2">{stats.shift2}</div><div>Классов во 2 смену</div></div>
                </div>
                {stats.classesWithoutTeacher > 0 && (
                  <div className="warning-box">⚠️ {stats.classesWithoutTeacher} класс(ов) без классного руководителя</div>
                )}
              </div>
            </div>
          )}
