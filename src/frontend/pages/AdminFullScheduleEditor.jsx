// src/frontend/pages/AdminFullSchedule.jsx
...
const ThemeToggle = () => {
  ...
  return (
    <button className={styles.themeBtn} onClick={() => setIsDark(!isDark)}>
      {isDark ? <FaSun /> : <FaMoon />}
...
const AdminFullSchedule = () => {
  ...
  // Формируем строки: день + номер урока
  const rows = useMemo(() => {
    const result = [];
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.topBar}>
        <button className={styles.backBtn} onClick={() => navigate(-1)}><FaArrowLeft /> Назад</button>
