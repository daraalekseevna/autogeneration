// src/frontend/pages/AdminScheduleEditor.jsx (исправленная версия с транспонированной таблицей)
...
import { 
  ...
  FaEye as FaEyeIcon, FaSun, FaMoon, FaFileExcel
} from 'react-icons/fa';
...
const AdminScheduleEditor = () => {
  ...
  // Уроки для первой и второй смены
  ...
  // Генерация mock расписания для всех классов
  ...
  // Проверка конфликтов для всех классов
  ...
  // Экспорт в Excel - транспонированная таблица (дни в строках, классы в столбцах)
  const exportToExcel = () => {
    const classesToExport = filteredClasses();
    const shiftName = selectedShift === 'first' ? 'Первая смена' : 'Вторая смена';
    const workbook = XLSX.utils.book_new();
    
    // Создаем лист для каждого дня недели
    allDays.forEach(day => {
      const sheetData = [];
  ...
  // Печать - транспонированная таблица (дни в строках, классы в столбцах)
  const printSchedule = () => {
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Урок</th>
              <th>Время</th>
              ${classesToPrint.map(c => `<th class="class-header">${c.name} класс<br><span style="font-size:8px">${c.classTeacher?.split(' ')[0] || ''}</span></th>`).join('')}
            </tr>
          </thead>
          <tbody>
            ${allDays.map(day => {
  ...
  // Рендер матричного представления (оставляем без изменений)
  ...
  // Рендер компактного представления
